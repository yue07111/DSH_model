...
def config_dataset(config):
    if "cifar" in config["dataset"]:
        ...
        config["topK"] = 5000
        config["n_class"] = 21
    elif config["dataset"] in "nuswide_10":
        config["topK"] = 5000
        config["n_class"] = 10
    ...
    # modify data_path to your own path
    config["data_path"] = "/dataset/" + config["dataset"] + "/"
    if config["dataset"] == "nuswide_21":
        config["data_path"] = "/disks/sata4/huangyue/dataset/NUS-WIDE/"
    if config["dataset"] == "nuswide_10":
        config["data_path"] = "/disks/sata4/huangyue/dataset/NUS-WIDE/"
    if config["dataset"] == "coco":
        config['data_path']="/disks/sata4/huangyue/dataset/COCO_2014/" 
    if config["dataset"] == "mirflickr":
        config["data_path"]="/disks/sata4/huangyue/dataset/mirflickr25k/mirflickr/"   
    if config["dataset"] in ["nuswide_21_m", "nuswide_81_m"]:
        config["data_path"] = "/dataset/nus_wide_m/"
    if config["dataset"] == "voc2012":
        config["data_path"] = "/dataset/"
    config["data"] = {
        "train_set": {"list_path": "/disks/sata4/huangyue/hy_DSH/DeepHash-hw/data/" + config["dataset"] + "/train.txt", "batch_size": config["batch_size"]},
        "database": {"list_path": "/disks/sata4/huangyue/hy_DSH/DeepHash-hw/data/" + config["dataset"] + "/database.txt", "batch_size": config["batch_size"]},
        "test": {"list_path": "/disks/sata4/huangyue/hy_DSH/DeepHash-hw/data/" + config["dataset"] + "/test.txt", "batch_size": config["batch_size"]}}
    return config
...
def cifar_dataset(config):
    ...
    cifar_dataset_root = '/disks/sata4/huangyue/dataset/cifar/'
    # Dataset
...
def validate(config, Best_mAP, test_loader, dataset_loader, net, bit, epoch, num_dataset):
    ...
    # Calculate binary codes and labels for the test set
    # print("calculating test binary code......")
    tst_binary, tst_label = compute_result(test_loader, net, device=device)

    # Calculate binary codes and labels for the training set
    ...
    # Save the model and binary codes if current mAP is the best so far
    if mAP > Best_mAP:
        Best_mAP = mAP
        if "save_path" in config:
            save_path = os.path.join(config["save_path"], f'{config["dataset"]}_{config["optimizer"]["type"].__name__}_{str(bit)}bits_{config["alpha"]}_{mAP}')
            #save_path = os.path.join(config["save_path"], f'{config["dataset"]}_{config["optimizer"].type}_{bit}bits_{mAP}')
